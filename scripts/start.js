    ...
    .then((result) => {
      ...
      http
        .createServer((req, res) => {
          if (req.url.match(/static\/dependencysearch/)) {
            // We need to remove the plugin "sub-directory". This is not part of the build output,
            // but is dynamically added by SonarQube. We build: `admin.js`, but
            // SonarQube serves this under `/static/dependencysearch/admin.js`.
            req.url = req.url.replace('static/dependencysearch/', '');
